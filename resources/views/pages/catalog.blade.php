                    ...
                    <div class="catalog-sidebar">
                        <div class="catalog-sidebar__search">
                            <form action="{{ route('category.index') }}" method="GET">
                                <input type="text" name="search" class="catalog-sidebar__input" placeholder="Поиск по товарам" value="{{ request()->input('search') }}">
                                <button type="submit" class="catalog-sidebar__button">
                        ...
                        <div class="catalog-sidebar__category">
                            ...
                            <div style="border: 1px solid #c9c9c9; padding: 10px; border-radius: 10px;">
                                <div style="display: flex; flex-direction: column; gap: 1rem;">
                                    <a href="?" class="catalog-sidebar__link" style="border-bottom: 1px solid #333; padding-bottom: .5rem; ">Все</a>
                                    @foreach($category as $item)
                                        <a href="?category={{$item->id}}" class="catalog-sidebar__link">{{$item->name}}</a>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <div class="catalog-sidebar__category">
                            <h3 class="catalog-sidebar__title">Сортировка</h3>
                                <form action="{{ route('category.index') }}" method="GET">
                                    <div class="select">
                                    <select name="sort_by" class="catalog-sidebar__list">
                                        <option value="price_asc" class="catalog-sidebar__item">От дешевого к дорогому</option>
                                        <option value="price_desc" class="catalog-sidebar__item">От дорогого к дешевому</option>
                                    </select>
                                    </div>
                                    <button type="submit" class="basket-status--buy">Сортировать</button>
                                </form>
                        </div>
